...
static void kernel_mapped_dso(struct dso *p)
{
    ...
    readelf_(p->name);
//     printf("testing if %p != %d\n", program_hdr, (size_t)ldso.phdr);
// 	if (program_hdr != (size_t)ldso.phdr) {}
//     printf("if %d (%d) (%d) != %d\n", getaux(AT_PHDR), aux[AT_PHDR], program_hdr, (size_t)ldso.phdr);
// 	if (program_hdr != (size_t)ldso.phdr) {
// 		size_t interp_off = 0;
// 		size_t tls_image = 0;
// 		/* Find load address of the main program, via AT_PHDR vs PT_PHDR. */
// 		Phdr *phdr = app.phdr = program_hdr;
// 		app.phnum = _elf_header->e_phnum;
// 		app.phentsize = _elf_header->e_phentsize;
// 		for (i=_elf_header->e_phnum; i; i--, phdr=(void *)((char *)phdr + _elf_header->e_phentsize)) {
// 			if (phdr->p_type == PT_PHDR)
// 				app.base = (void *)(program_hdr - phdr->p_vaddr);
...
// 			}
    printf("p->name = %s\n", p->name);
	Phdr *ph = _elf_program_header;
	for (cnt = _elf_header->e_phnum; cnt--; ph = (void *)((char *)ph + _elf_header->e_phentsize)) {
...
static void decode_vec_(size_t *v, size_t *a, size_t cnt)
{
    ...
    for (; v[0]; v+=2) {
        ...
        if (AUXV_TYPE == "CHAR*") {
            ...
            printf("a[%s] (%d) = %p\n", AUXV_NAME, v[0], v[1] );
        } else printf("a[%s] (%d) = %p (UNKNOWN TYPE)\n", AUXV_NAME, v[0], v[1] );
        if (v[0]-1<cnt-1) {
            a[0] |= 1UL<<v[0];
            a[v[0]] = v[1];
        }
...
int readelf_(const char * name) {
    ...
    if (array == MAP_FAILED) {
        ...
        printf ("map succeded with address: 0x%08x\n", &array);
    }
...
void init_structs(const char *name) {
    printf("----------------------------------------------------------------------->called %s() at line %d from %s\n", __func__, __LINE__, __FILE__);
    #ifndef AUX_CNT
        #define AUX_CNT 32
    #endif
...
//             printf("setting app.name to (char *)aux[AT_EXECFN] (%s)\n", (char *)aux[AT_EXECFN]);
// 			app.name = (char *)aux[AT_EXECFN];
            printf("setting app.name to name (%s)\n", name);
			app.name = name;
        }
		else {
            printf("setting app.name to name (%s)\n", name);
