...
#include "dynlink.h"
#include <assert.h>
void * getaux(unsigned long type);
void abort_() {
...
char * program_hdr;
int readelf_(const char * name );
void init_structs(const char *filename____);
static void decode_vec_(size_t *v, size_t *a, size_t cnt);
#define decode_vec decode_vec_
#define PRINT_OPAQUE_STRUCT(p)  print_mem((p), sizeof(*(p)))

int print_mem(void const *vp, size_t n)
{
    printf("----------------------------------------------------------------------->called %s() at line %d from %s\n", __func__, __LINE__, __FILE__);
    int struct_contents = 0;
    unsigned char const *p = vp;
    for (size_t i=0; i<n; i++)
        if (!p[i] == 0x0) {
            printf("struct index %d = %08x\n", i, p[i]);
            struct_contents = 1;
            putchar('\n');
        }
    if (struct_contents == 0) {
...
{
    printf("----------------------------------------------------------------------->called %s() at line %d from %s\n", __func__, __LINE__, __FILE__);
    printf("attempting to compare %s with %s\n", l, r);
	for (; *l==*r && *l; l++, r++);
    printf("returning \n%c\n %c \n- \n%c\n", *(unsigned char *)l - *(unsigned char *)r, *(unsigned char *)l,  *(unsigned char *)r);
	...
	for (; rel_size; rel+=stride, rel_size-=stride*sizeof(size_t)) {
		...
		case REL_TLSDESC:
			...
			if (runtime && def.dso->tls_id >= static_tls_cnt) {
				struct td_index *new = mallocb(sizeof *new);
				if (!new) {
					error(
...
static void *map_library(int fd, struct dso *dso)
{
	...
	if (l<sizeof *eh || (eh->e_type != ET_DYN && eh->e_type != ET_EXEC)) {
        printf("HAS NO ET_DYN OR ET_EXEC ( l<sizeof *eh = %d (%d<%d), (eh->e_type != ET_DYN && eh->e_type != ET_EXEC) = %d ((%d != %d && %d != %d)))\n", l<sizeof *eh, l, sizeof *eh, (eh->e_type != ET_DYN && eh->e_type != ET_EXEC), eh->e_type, ET_DYN, eh->e_type, ET_EXEC);
		goto noexec;
    }
	phsize = eh->e_phentsize * eh->e_phnum;
	if (phsize > sizeof buf - sizeof *eh) {
		allocated_buf = mallocb(phsize);
	...
	if (DL_FDPIC && !(eh->e_flags & FDPIC_CONSTDISP_FLAG)) {
		dso->loadmap = callocb(1, sizeof *dso->loadmap
...
static int fixup_rpath(struct dso *p, char *buf, size_t buf_size)
{
	...
	if (libc.secure && *origin != '/')
		return 0;
	p->rpath = mallocb(strlen(p->rpath_orig) + n*l + 1);
	if (!p->rpath) return -1;
...
static void decode_dyn(struct dso *p)
{
    printf("----------------------------------------------------------------------->called %s() at line %d from %s\n", __func__, __LINE__, __FILE__);
	size_t dyn[DYN_CNT];
    if (p->dynv == NULL) printf ("p->dynv is NULL\n");
    printf("p->dynv = %p\n", p->dynv);
//     assert(p->dynv == NULL);
...
static struct dso *load_library(const char *name, struct dso *needed_by)
{
	...
	ldso.name = "/lib/ld-musl-x86_64.so.1";
	printf("comparing %s with %s\n", name, ldso.name); // comparing ./libtest_lib.so with /lib/ld-musl-x86_64.so.1
// 	if (!strcmp(name, ldso.name)) is_self = 1;
	printf("strcmpb(%s, \"libc.so.6\") = %d\n", name, strcmpb(name, "libc.so.6"));
    if (strcmpb(name, "libc.so.6") == 0 || strcmpb(name, "ld-musl-x86_64.so.1") == 0) {
        is_self = 0;
    }
    printf("is_self = %d\n", is_self);
	if (is_self) {
	...
	if (strchr(name, '/')) {
		...
		fd = -1;
		if (env_path) {
            printf("attempting to open %s (env_path = %s)\n", name, env_path);
		...
		for (p=needed_by; fd == -1 && p; p=p->needed_by) {
			...
			if (p->rpath) {
                printf("attempting to open %s (p->rpath = %s)\n", name, p->rpath);
		...
		if (fd == -1) {
            ...
            printf("attempting to open %s (sys_path = %s)\n", name, sys_path);
...
static void load_deps(struct dso *p)
{
	...
	for (; p; p=p->next) {
		for (i=0; p->dynv[i]; i+=2) {
			...
			if (runtime) {
				tmp = reallocb(*deps, sizeof(*tmp)*(ndeps+2));
...
static void prepare_lazy(struct dso *p)
{
	...
	p->lazy = callocb(n, 3*sizeof(size_t));
	if (!p->lazy) {
...
static void decode_vec_(size_t *v, size_t *a, size_t cnt)
{
    ...
    void * AUXV_TYPE;
    char * AUXV_NAME;
    printf("preparing second argument\n");
    for (i=0; i<cnt; i++) {
        a[i] = 0;
    }
    printf("searching first argument for vectors\n");
    // v[0] appears to be the index, v[1] appears to hold the contents of the current v[0]
    for (; v[0]; v+=2) {
        switch(*v)
        {
                case 0:
        ...
        if (AUXV_TYPE == "CHAR*") {
            ...
            printf("a[%s] (%d) = %p\n", AUXV_NAME, v[0], v[1] );
        } else printf("a[%s] (%d) = %p (decimal value: %d) (UNKNOWN TYPE)\n", AUXV_NAME, v[0], v[1], v[1] );
        if (v[0]-1<cnt-1) {
            a[0] |= 1UL<<v[0];
            a[v[0]] = v[1];
        }
...
//     printf("__environ[0] = \n");
//     printf("%s\n", __environ[0]);
//     printf("saved_environ[0] = \n");
//     printf("%s\n", __environ[0]);
    size_t iii;
    char **p = &__environ[-2];
    for (iii = 1; iii != *(size_t*)(p-1); iii++) {
        p--;
    }
    argc = (int)iii;
    argv = p;
...
int readelf_(const char * name) {
    ...
    if (array == MAP_FAILED) {
        ...
        printf ("mapped %s successfully with address: 0x%08x\n", name, &array);
    }
    Elf64_Ehdr *_elf_header;
    Elf64_Phdr *_elf_program_header;
    Elf64_Shdr *_elf_symbol_table;
	...
	if (l<sizeof *eh || (eh->e_type != ET_DYN && eh->e_type != ET_EXEC)) {
        printf("HAS NO ET_DYN OR ET_EXEC ( l<sizeof *eh = %d (%d<%d), (eh->e_type != ET_DYN && eh->e_type != ET_EXEC) = %d ((%d != %d && %d != %d)))\n", l<sizeof *eh, l, sizeof *eh, (eh->e_type != ET_DYN && eh->e_type != ET_EXEC), eh->e_type, ET_DYN, eh->e_type, ET_EXEC);
		abort_();
    } else printf("HAS VALID ET_DYN OR ET_EXEC ( l<sizeof *eh = %d (%d<%d), (eh->e_type != ET_DYN && eh->e_type != ET_EXEC) = %d ((%d != %d && %d != %d)))\n", l<sizeof *eh, l, sizeof *eh, (eh->e_type != ET_DYN && eh->e_type != ET_EXEC), eh->e_type, ET_DYN, eh->e_type, ET_EXEC);
...
//                 dl_iterate_phdr(callback, NULL);
            ...
            switch(_elf_program_header[i].p_type)
            {
                ...
                case PT_LOAD:
                    printf("PT_LOAD\t\t/* Loadable program segment *\n");
                    /* we only need the 1st PT_LOAD segment, so stop if more then one is read */
                    if (!p_a) {
                        printf("p_flags;\t\t/* Segment flags */\t\t= 0x%08x\np_offset;\t\t/* Segment file offset */\t= 0x%08x\np_vaddr;\t\t/* Segment virtual address */\t= 0x%08x\np_paddr;\t\t/* Segment physical address */\t= 0x%08x\np_filesz;\t\t/* Segment size in file */\t= 0x%08x\np_memsz;\t\t/* Segment size in memory */\t= 0x%08x\np_align;\t\t/* Segment alignment */\t\t= 0x%08x\n\n\n", _elf_program_header[i].p_flags, _elf_program_header[i].p_offset, _elf_program_header[i].p_vaddr, _elf_program_header[i].p_paddr, _elf_program_header[i].p_filesz, _elf_program_header[i].p_memsz, _elf_program_header[i].p_align);
                        program_hdr = _elf_header->e_phoff+_elf_program_header[i].p_paddr;
                        printf("program_hdr = %p\n", program_hdr);
                        p_a = 1;
                    }
                    break;
                case PT_DYNAMIC:
...
_elf_header->e_shoff = 0x%08x)\n",\
        ...
        char * print_section_headers_(char * sourcePtr, Elf64_Ehdr * eh, Elf64_Shdr sh_table[]) {
            ...
            for(i=0; i<eh->e_shnum; i++) { // will fail untill section header table can be read
                printf("\t%03d ", i);
                printf("0x%08x ", _elf_symbol_table[i].sh_offset);
                printf("0x%08x ", _elf_symbol_table[i].sh_addr);
                printf("0x%08x ", _elf_symbol_table[i].sh_size);
                printf("%4d ", _elf_symbol_table[i].sh_addralign);
                printf("0x%08x ", _elf_symbol_table[i].sh_type);
                printf("0x%08x ", _elf_symbol_table[i].sh_flags);
        ...
        void print_elf_symbol_table(char * arrayc, Elf64_Ehdr eh, Elf64_Shdr sh_table[], uint64_t symbol_table)
        {
            ...
            */
            ...
            for(i=0; i< symbol_count; i++) {
                printf("address: = 0x%08x ", sym_tbl[i].st_value);
                printf("binding: ");
...
// //             char * mempage[1];
// //             int len;
// //             stream__("/proc/self/maps", &mempage[0], &len, -1);
// //             printf("mempage[0] (original) =");
// //             const char * original = print_quoted_string(mempage[0], len, QUOTE_FORCE_HEX|QUOTE_OMIT_LEADING_TRAILING_QUOTES, "return");
// //             printf("%s\n", original);
//             printf("dl_iterate_phdr =\n");
//             dl_iterate_phdr(callback, NULL);
// 
// //             char * mempagemap;
// //             int lenb;
// //             stream__("/proc/self/pagemap", &mempage, &lenb, -1);
// //             printf("mempagemap =\n", mempage);
//             
// //             print_maps();
    ...
    array = NULL;
    _elf_header = NULL;
    _elf_program_header = NULL;
    _elf_symbol_table = NULL;
    return 0;
}

void init_structs(const char *filename____) {
    printf("----------------------------------------------------------------------->called %s() at line %d from %s\n", __func__, __LINE__, __FILE__);
    readelf_(filename____);
    #ifndef AUX_CNT
        #define AUX_CNT 32
    #endif
    printf("retrieving current aux\n");
    getaux(AT_EXECFN);
    getaux(AT_PLATFORM);
    getaux(AT_PHDR);
    getaux(AT_SYSINFO_EHDR);
    getaux(AT_HWCAP);
    getaux(AT_BASE);
    getaux(AT_ENTRY);
    getaux(AT_PHNUM);
...
// 	__environ = envp;
    ...
    printf("decode_vec(%zu, %zu, %zu);\n", *auxv, *aux, AUX_CNT);
	decode_vec_(auxv, aux, AUX_CNT);
    printf("EXECFN = %s\n", aux[AT_EXECFN]);
